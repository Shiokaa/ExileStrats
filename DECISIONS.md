...
| D34 | **Auth = Supabase Auth**, deux méthodes en V1 : **Discord OAuth** (provider intégré) **+ email/mot de passe** (page `/auth`, onglets Sign in / Sign up). Le **login Path of Exile** (OAuth `account:profile`) reste une **addition future**, pas un remplacement, à brancher quand/si GGG approuve l'app (`oauth@grindinggear.com`, discrétionnaire, rejette les demandes IA → demande écrite à la main). Sécurité (doc Supabase) : **`getUser()`** pour toute autorisation (jamais `getSession()` côté serveur), **anon key** seule au client (jamais `service_role`), PKCE, middleware (`proxy`) de refresh ; mots de passe gérés par Supabase (hash/stockage/règles), jamais par nous. Supabase = auth seule ; Prisma = donnée. `Strategy.authorId` (UUID user) à venir avec la persistance de Créer | Discord + email débloquent l'édition tout de suite sans dépendre de l'approbation GGG ; PoE login reste l'objectif thématique. Affine D4 (PoE OAuth = reporté, pas interdit à vie) | Actif |
