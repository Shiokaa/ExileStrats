...
| D34 | **Auth = stratégie hybride**. V1 : **Supabase Auth via Discord** (provider intégré) — conforme à D4. Le **login Path of Exile** (OAuth `account:profile`) est une **addition future**, pas un remplacement, à brancher quand/si GGG approuve l'app (`oauth@grindinggear.com`, discrétionnaire, rejette les demandes IA → demande écrite à la main). Sécurité (doc Supabase) : **`getUser()`** pour toute autorisation (jamais `getSession()` côté serveur), **anon key** seule au client (jamais `service_role`), PKCE, middleware de refresh. Supabase = auth seule ; Prisma = donnée. `Strategy.authorId` (UUID user) à venir avec la persistance de Créer | Discord débloque l'édition tout de suite sans dépendre de l'approbation GGG ; PoE login reste l'objectif thématique. Affine D4 (PoE OAuth = reporté, pas interdit à vie) | Actif |
